(function () {
	...
	async function upload$1(filename, content, options) {
		...
		function sendRequest(path, method, body) {
			const headers = {
				[AUTHORIZATION_HEADER$1]: authorization
			};
			if (body) {
	...
	async function downloadPageForeground(taskId, filename, content, tabId, foregroundSave) {
		const serializer = getSerializer({ filename, taskId, foregroundSave, content: await content.arrayBuffer() });
				...
				if (options.autoSaveExternalSave) {
					...
					if (!skipped) {
						...
						if (options.saveToGDrive) {
							...
							await (await saveToGitHub(message.taskId, encodeSharpCharacter(pageData.filename), content, options.githubToken, options.githubUser, options.githubRepository, options.githubFolder, options.githubBranch, {
								filenameConflictAction: options.filenameConflictAction
							})).pushPromise;
